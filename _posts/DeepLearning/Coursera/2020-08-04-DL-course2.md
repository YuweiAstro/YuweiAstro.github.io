...
in which $\parallel w^{[l]} \parallel^2\_2 = \sum\limits_{i=1}^{n^l}\sum\limits_{j=1}^{n^{[l-1]}} (w_{i,j}^{[l]})^2$ is the L2 regularization, and the value is the squared Euclidean norm; the matrix norm  is called the "Frobenius norm". And thus 
$J\_{regularized} = \small \underbrace{-\frac{1}{m} \sum\limits\_{i = 1}^{m} \large{(}\small y^{(i)}\log\left(a^{\[L\](i)}\right) + (1-y^{(i)})\log\left(1- a^{\[L\](i)}\right) \large{)} }\_\text{cross-entropy cost} + \underbrace{\frac{1}{m} \frac{\lambda}{2} \sum\limits_l\sum\limits_k\sum\limits_j W_{k,j}^{[l]2} }_\text{L2 regularization cost}$
